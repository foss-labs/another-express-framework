...
import {
  ...
  injectable as inverseInjectable,
  inject as inverseInject,
  decorate,
...
class Container {
  ...
  register<T>(provider: Provider<T>): void {
    if (typeof provider === "function") {
      ...
      this.inversifyContainer.bind<(...args: any[]) => T>(provider.provide as Token<(...args: any[]) => T>).toFactory(() => provider.useFactory);
  ...
  private applyScope<T>(binding: BindInWhenOnFluentSyntax<T>, scope: Scope): void {
    switch (scope) {
      case "Singleton":
